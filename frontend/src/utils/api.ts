...
export function me(theme?: Theme, hasSidebarOpen?: boolean, customInstructions?: string) {
    if (theme === undefined && hasSidebarOpen === undefined && customInstructions === undefined) {
        return apiFetch("me/")
    } else {
        return apiFetch("me/", {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ theme, has_sidebar_open: hasSidebarOpen, custom_instructions: customInstructions })
        })
    }
}
